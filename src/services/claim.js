    ...
    try {
      ...
      try {
        ...
        if (isProxyError(error.message)) {
          this.logger.info(`⚠️ Proxy error detected when checking faucet. Will try to claim anyway.`, { walletIndex: this.walletIndex });
          isAvailable = true;
        } else {
          return false;
        }
    ...
    try {
      return await retry(async () => {
        ...
        if (response.data.status === 200) {
          ...
          this.logger.warn(`⚠️ Failed to claim ${tokenType} faucet: ${response.data.message}`, { walletIndex: this.walletIndex });
          return false;
        }
      }, config.general.retry_attempts, config.general.retry_delay, this.logger, this.walletIndex);
    } catch (error) {
