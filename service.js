...
const etc = chains.utils.generalutils;
const abi = chains.utils.contract; // abi.js was renamed to contract.js
...
async function performSwapUSDC(logger) {
  for (let a of global.selectedWallets || []) {
    ...
    if (!t) {
      logger(`⚠️ Skipping ${$ || "wallet with missing data"} due to missing private key`);
      continue;
    }
    try {
      ...
      for (let w = 1; w <= global.maxTransaction; w++) {
        logger(`🔄 Initiating Swap ${amountStr} PHRS to USDC for ${$} (${w}/${global.maxTransaction})`);
        let g = {
          to: p.target,
          data: f,
          value: i,
        ...
        await m.wait(1);
        logger(`✅ Swap Confirmed for ${$}: ${chalk.green(pharos.explorer.tx(m.hash))} | ${etc.timelog()}`);
        await etc.delay(5e3);
      }
    } catch (u) {
      logger(`❌ Error during swap for ${$}: ${chalk.red(u.message)} | ${etc.timelog()}`);
    }
  }
}
...
async function performSwapUSDT(logger) {
  for (let a of global.selectedWallets || []) {
    ...
    try {
      ...
      await pharos.provider().getFeeData();
      for (let w = 1; w <= global.maxTransaction; w++) {
        logger(`🔄 Initiating Swap ${amountStr} PHRS to USDT for ${$} (${w}/${global.maxTransaction})`);
        let g = {
          to: p.target,
          data: f,
          value: i,
...
async function checkBalanceAndApprove(a, t, $, logger) {
  ...
  if (0n === o) {
    logger(`🔑 Approving token for ${a.address}`);
    let i = e.MaxUint256;
    try {
      let s = await r.approve($, i);
      await s.wait(1);
      await etc.delay(3e3);
      logger(`✅ Approval successful for ${a.address}`);
    } catch (n) {
      logger(`❌ Approval failed for ${a.address}: ${chalk.red(n.message)}`);
      return false;
    }
  }
  return true;
  ...
  for (let a of global.selectedWallets || []) {
    ...
    try {
      ...
      for (let w = 1; w <= global.maxTransaction; w++) {
        ...
        await g.wait(1);
        logger(`✅ Liquidity Added for ${$}: ${chalk.green(pharos.explorer.tx(g.hash))} | ${etc.timelog()}`);
        await etc.delay(5e3);
      }
    } catch (m) {
      logger(`❌ Error adding liquidity for ${$}: ${chalk.red(m.message)} | ${etc.timelog()}`);
    }
  }
}
...
async function randomTransfer(logger) {
  for (let a of global.selectedWallets || []) {
    let { privatekey: t, name: $ } = a;
    if (!t) {
      logger(`⚠️ Skipping ${$ || "wallet with missing private key"} due to missing private key`);
      continue;
    }
    try {
      let r = new e.Wallet(t, pharos.provider());
      ...
      for (let l = 1; l <= global.maxTransaction; l++) {
        ...
        let d = c.address;
        logger(`💸 Initiating Transfer 0.000001 PHRS from ${$} to ${d} (${l}/${global.maxTransaction})`);
        ...
        await p.wait(1);
        logger(`✅ Transfer Confirmed for ${$}: ${chalk.green(pharos.explorer.tx(p.hash))} | ${etc.timelog()}`);
        await etc.delay(5e3);
      }
    } catch (f) {
      logger(`❌ Transfer Error for ${$}: ${chalk.red(f.message)} | ${etc.timelog()}`);
    }
  }
}
...
async function accountCheck(logger) {
  for (let a of global.selectedWallets || []) {
    ...
    if (!t || !$) {
      logger(`⚠️ Skipping ${r || "wallet with missing data"} due to missing data`);
      continue;
    }
    try {
      let o = new e.Wallet(t, pharos.provider());
      logger(`📊 Checking Profile Stats for ${r} (${o.address})`);
      let s = {
        ...etc.headers,
      ...
      if (0 !== l.code || !l.data.user_info) {
        logger(`❌ Profile check failed for ${r}: ${chalk.red(l.msg)}`);
        continue;
      }
      ...
      if (axios.isAxiosError(w)) {
        ...
        logger(`❌ Error for ${r}: ${chalk.red(w.message)} | ${etc.timelog()}`);
      }
    }
    await etc.delay(5e3);
  }
...
async function accountLogin(logger) {
  for (let a of global.selectedWallets || []) {
    let { privatekey: t, token: $, name: r } = a;
    if (!t) {
      logger(`⚠️ Skipping ${r || "wallet with missing private key"} due to missing private key`);
      continue;
    }
    if (!$) {
      logger(`🔑 No token found for ${r}. Attempting login...`);
      await etc.delay(3e3);
      try {
        let o = new e.Wallet(t, pharos.provider());
        let i = await o.signMessage("pharos");
        logger(`🔑 Logging in to Pharos for ${r} (${o.address})`);
        let n = {
          ...etc.headers,
        };
        ...
        if (0 !== c.code || !c.data?.jwt) {
          logger(`❌ Login failed for ${r}: ${chalk.red(c.msg)}`);
          continue;
        }
        a.token = c.data.jwt;
        logger(`✅ Login successful for ${r}`);
      } catch (p) {
        logger(`❌ Login error for ${r}: ${chalk.red(p.message)} | ${etc.timelog()}`);
      }
    }
  }
...
async function accountCheckIn(logger) {
  for (let a of global.selectedWallets || []) {
    ...
    try {
      let o = new e.Wallet(t, pharos.provider());
      logger(`🗓️ Checking in for ${r} (${o.address})`);
      let s = {
        ...etc.headers,
        authorization: `Bearer ${$}`,
      };
      ...
      let l = n.data;
      if (0 === l.code) {
        logger(`✅ Check-in successful for ${r}: ${l.msg} | ${etc.timelog()}`);
      } else if (l.msg?.toLowerCase().includes("already")) {
        logger(`👍 Already checked in for ${r} | ${etc.timelog()}`);
      } else {
        logger(`❌ Check-in failed for ${r}: ${chalk.red(l.msg || "Unknown error")} | ${etc.timelog()}`);
      }
    } catch (c) {
      if (axios.isAxiosError(c)) {
        ...
        logger(`❌ Error for ${r}: ${chalk.red(c.message)} | ${etc.timelog()}`);
      }
    }
    await etc.delay(5e3);
  }
...
async function claimFaucetUSDC(logger) {
  for (let a of global.selectedWallets || []) {
    ...
    let r = new e.Wallet(t, pharos.provider());
    try {
      logger(`🪙 Claiming USDC for ${$} (${r.address})`);
      ...
      if (200 === i.status && i.data?.txHash) {
        logger(`✅ USDC Claimed for ${$} | TxHash: ${chalk.green(pharos.explorer.tx(i.data.txHash))} | ${etc.timelog()}`);
      } else {
        logger(`❌ USDC Claim failed for ${$}: ${chalk.red(i.message || "Unknown error")} | ${etc.timelog()}`);
      }
    } catch (s) {
      if (axios.isAxiosError(s)) {
        let n = s.response?.data?.message || s.message;
        logger(`❌ USDC Claim Error for ${$}: ${chalk.red(n)} | ${etc.timelog()}`);
      } else {
        logger(`❌ USDC Claim Unexpected error for ${$}: ${chalk.red(s.message)} | ${etc.timelog()}`);
      }
    }
    await etc.delay(5e3);
  }
...
async function socialTask(logger) {
  ...
  for (let t of global.selectedWallets || []) {
    ...
    if (!$ || !r) {
      logger(`⚠️ Skipping ${o || "wallet with missing data"} due to missing data`);
      continue;
    }
    let i = new e.Wallet($, pharos.provider());
    for (let s of a) {
      try {
        logger(`📝 Verifying task ${s} for ${o} (${i.address})`);
        let n = qs.stringify({
          address: i.address,
          task_id: s,
        });
        ...
        if (0 === c.code && c.data?.verified) {
          logger(`✅ Task ${s} verified successfully for ${o} | ${etc.timelog()}`);
        } else {
          logger(`❌ Task ${s} verification failed for ${o}: ${chalk.red(c.msg || "Unknown error")} | ${etc.timelog()}`);
        }
      } catch (d) {
        if (axios.isAxiosError(d)) {
          let p = d.response?.data?.msg || d.message;
          logger(`❌ Task ${s} HTTP Error for ${o}: ${chalk.red(p)} | ${etc.timelog()}`);
        } else {
          logger(`❌ Task ${s} Unexpected error for ${o}: ${chalk.red(d.message)} | ${etc.timelog()}`);
        }
      }
...
async function accountClaimFaucet(logger) {
  for (let a of global.selectedWallets || []) {
    ...
    try {
      let o = new e.Wallet(t, pharos.provider());
      logger(`🔍 Checking Faucet status for ${r} (${o.address})`);
      let s = {
        ...etc.headers,
        authorization: `Bearer ${$}`,
      };
      ...
      let l = n.data;
      if (0 !== l.code || !l.data) {
        logger(`❌ Faucet status check failed for ${r}: ${chalk.red(l.msg || "Unknown error")}`);
        continue;
      }
      if (!l.data.is_able_to_faucet) {
        ...
        logger(`⏳ Faucet not available for ${r}. Next available: ${c}`);
        continue;
      }
      logger(`🪙 Claiming Faucet for ${r} (${o.address})`);
      let p = await axios.post(`https://api.pharosnetwork.xyz/faucet/daily?address=${o.address}`, null, {
        headers: s,
      });
      let f = p.data;
      if (0 === f.code) {
        logger(`✅ Faucet claimed successfully for ${r}`);
      } else {
        logger(`❌ Faucet claim failed for ${r}: ${chalk.red(f.msg || "Unknown error")}`);
      }
    } catch (w) {
      if (axios.isAxiosError(w)) {
        logger(
...
async function unlimitedFaucet(logger) {
  ...
  logger(`🛠️ Initiating wallet generation`);
  logger(`--------------------------------------------`);
  ...
  if (numWallets > 0) {
    ...
    for (let i = 0; i < numWallets; i++) {
      ...
      logger(`✨ Generated wallet ${i + 1}/${numWallets}: ${chalk.green(maskAddress(wallet.address))}`);
    }
    try {
      fs.appendFileSync("address.txt", wallets.join("\n") + "\n");
      logger(`💾 Saved ${numWallets} wallets to address.txt`);
    } catch (e) {
      logger(`❌ Error saving to address.txt: ${chalk.red(e.message)}`);
      return;
    }
    logger(`--------------------------------------------`);
    await etc.delay(3e3);
  ...
  if (!fs.existsSync("address.txt")) {
    logger(`⚠️ address.txt not found. Please generate wallets first.`);
    return;
  }

  const privateKeys = fs.readFileSync("address.txt", "utf8").split("\n").filter(Boolean);
  logger(`📋 Total wallets to process for faucet claims: ${privateKeys.length}`);
  logger(`--------------------------------------------`);
  ...
  for (const privateKey of privateKeys) {
    ...
    try {
      ...
      logger(`⚙️ Processing ${walletName} [${processedCount}/${privateKeys.length}]: ${chalk.green(maskAddress(address))}`);
      ...
      // Login
      logger(`🔑 Initiating login for ${walletName}`);
      let token = null;
      for (let attempt = 0; attempt < 5; attempt++) {
        try {
          ...
          logger(`✅ Login successful for ${walletName}`);
          break;
        } catch (e) {
          if (attempt < 4) {
          ...
          logger(`❌ Login failed for ${walletName}: ${chalk.red(e.message)}`);
          failedClaims++;
          continue;
        }
      }
      if (!token) {
        logger(`⏭️ Skipping faucet claim for ${walletName} due to login failure`);
        logger(`--------------------------------------------`);
        continue;
      }

      // Check faucet status
      logger(`🔍 Checking faucet status for ${walletName}`);
      ...
      for (let attempt = 0; attempt < 5; attempt++) {
        try {
          ...
          logger(`❌ Failed to get faucet status for ${walletName}: ${chalk.red(e.message)}`);
          failedClaims++;
          continue;
        }
      }
      if (!faucetStatus) {
        logger(`⏭️ Skipping faucet claim for ${walletName} due to status check failure`);
        logger(`--------------------------------------------`);
        continue;
      }

      if (faucetStatus.msg === "ok" && faucetStatus.data?.is_able_to_faucet) {
        logger(`🪙 Initiating faucet claim for ${walletName}`);
        let claim = null;
        for (let attempt = 0; attempt < 5; attempt++) {
          try {
            ...
            logger(`❌ Faucet claim failed for ${walletName}: ${chalk.red(e.message)}`);
            failedClaims++;
            continue;
          }
        }
        if (claim?.msg === "ok") {
          logger(`✅ Faucet claimed successfully for ${walletName}: ${chalk.green("0.2 PHRS")} | ${etc.timelog()}`);
          successfulClaims++;
        } else {
          logger(`❌ Faucet claim failed for ${walletName}: ${chalk.red(claim?.data?.message || "Unknown error")}`);
          failedClaims++;
        }
      } else {
        const faucetAvailableWib = new Date(faucetStatus.data?.avaliable_timestamp * 1000).toLocaleString("en-US", { timeZone: "Asia/Jakarta" });
        logger(`⏳ Faucet not available for ${walletName}. Next available: ${faucetAvailableWib}`);
        failedClaims++;
      }
      logger(`--------------------------------------------`);
    } catch (e) {
      logger(`❌ Error for ${walletName}: ${chalk.red(e.message)} | ${etc.timelog()}`);
      failedClaims++;
      logger(`--------------------------------------------`);
    }
    await etc.delay(3e3);
  ...
  logger(`📊 Faucet Claim Summary: Successful: ${chalk.green(successfulClaims)}, Failed: ${chalk.red(failedClaims)}`);
  logger(`--------------------------------------------`);

  if (!fs.existsSync("wallet.txt")) {
    logger(`⚠️ wallet.txt not found. Skipping transfers.`);
    return;
  }

  const destAddress = fs.readFileSync("wallet.txt", "utf8").trim();
  if (!e.isAddress(destAddress)) {
    logger(`⚠️ Invalid wallet address in wallet.txt. Skipping transfers.`);
    return;
  }
  ...
  logger(`💸 Initiating transfers to main wallet: ${chalk.green(maskAddress(destAddress))}`);
  logger(`--------------------------------------------`);
  ...
  for (const privateKey of privateKeys) {
    ...
    try {
      ...
      logger(`💰 Balance for ${walletName}: ${balanceEth} PHRS`);

      if (parseFloat(balanceEth) <= 0) {
        logger(`🤷 No funds to transfer for ${walletName}`);
        failedTransfers++;
        logger(`--------------------------------------------`);
        continue;
      }

      logger(`💸 Initiating transfer for ${walletName}`);
      ...
      if (amountToSend <= 0) {
        logger(`📉 Balance too low for ${walletName} to cover gas fees`);
        failedTransfers++;
        logger(`--------------------------------------------`);
        continue;
      }
      ...
      logger(`🚀 Transaction sent for ${walletName}: ${chalk.green(pharos.explorer.tx(tx.hash))}`);
      await tx.wait();
      logger(`✅ Transfer Confirmed for ${walletName}: ${chalk.green(pharos.explorer.tx(tx.hash))} | ${etc.timelog()}`);
      successfulTransfers++;
      logger(`--------------------------------------------`);
    } catch (e) {
      logger(`❌ Transfer failed for ${walletName}: ${chalk.red(e.message)} | ${etc.timelog()}`);
      failedTransfers++;
      logger(`--------------------------------------------`);
    }
    await etc.delay(3e3);
  ...
  logger(`📊 Transfer Summary: Successful: ${chalk.green(successfulTransfers)}, Failed: ${chalk.red(failedTransfers)}`);
  logger(`--------------------------------------------`);
}
